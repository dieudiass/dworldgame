...
<head>
    <meta charset="UTF-8">
    <title>Admin: Add An AccountType</title>
    ...
    <div class="row profile">
        ...
        <div class="col-md-9">
            <div class="profile-content">
                <h4 class="text-center">Add An Account Type</h4>
                <?php
                ...
                if(isset($_POST["add-type"])){
                    ...
                    if(isset($_SESSION["user"])){
                        ...
                        if($_POST["add-type"] == $_SESSION["user"]["token"]){
                            //Create errors
                            $errors = array();
                            //Clean the request
                            ...
                            if(strlen($name) < 3){
                                $errors[] = "The name of the account type is too short, it must be more than two letters.";
                            }
                            //Check the length of the description
                            if(strlen($desc) < 6){
                                $errors[] = "The description of the account type is too short, it must be more than five letters.";
                            }
                            ...
                            if(empty($errors)){
                                ...
                                //Check if the type was created successfully
                                if($type->create()){
                                    ?>
                                    <div class="alert alert-success"><?php echo $name; ?> account type was added succesfully!</div>
                                    <?php
                                }else{
                                    ?>
                                    <div class="alert alert-warning">There was a problem adding the type <?php echo $name; ?>, please try again later. If this problem persists, please report this error.</div>
                                    <?php
                                }
                            }else{
                                //There were errors
                ...
                ?>
                <form method="post" action="" id="add-type" role="form">
                    ...
                    <div class="form-group">
                        <input type="hidden" name="add-type" value="<?php if(isset($_SESSION["user"]["token"])){ echo $_SESSION["user"]["token"]; } ?>">
                        <button name="submit" type="submit" class="btn btn-info btn-block" id="submit">Add Account Type</button>
                    </div>
                </form>
            </div>
        </div>
